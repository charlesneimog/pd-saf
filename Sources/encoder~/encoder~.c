...
t_int *encoder_tilde_perform(t_int *w) {
    ...
    for (int ch = 0; ch < x->nSH; ch++) {
        x->outs[ch] = (t_sample *)(w[3 + x->num_sources + ch]);
    }

    ambi_enc_process(x->hAmbi, (const float *const *)x->accumIn, x->outs, x->num_sources, x->nSH,
                     x->frameSize);
...
void *encoder_tilde_new(t_symbol *s, int argc, t_atom *argv) {
    ...
    x->frameSize = ambi_enc_getFrameSize();

    // accum buffer
    x->accumIn = (t_sample **)getbytes(x->num_sources * sizeof(t_sample *));
