...
typedef struct _decoder_tilde {
    ...
    t_sample **accumIn; // One buffer per spherical harmonic channel (nSH channels)
    int accumIndex;
} t_decoder_tilde;
...
t_int *decoder_tilde_perform(t_int *w) {
    ...
    // Set up input pointers for the nSH channels:
    for (int ch = 0; ch < x->nSH; ch++) {
        x->ins[ch] = (t_sample *)(w[3 + ch]);
    }
    // Set up output pointers for the loudspeaker channels:
    for (int ch = 0; ch < x->num_loudspeakers; ch++) {
        x->outs[ch] = (t_sample *)(w[3 + x->nSH + ch]);
    }

    int processed = 0;
    while (processed < blockSize) {
        int samplesNeeded = x->frameSize - x->accumIndex;
        int samplesAvailable = blockSize - processed;
        int samplesToCopy = (samplesAvailable < samplesNeeded) ? samplesAvailable : samplesNeeded;
        for (int ch = 0; ch < x->nSH; ch++) {
            memcpy(x->accumIn[ch] + x->accumIndex, x->ins[ch] + processed,
                   samplesToCopy * sizeof(t_sample));
        }
        x->accumIndex += samplesToCopy;
        processed += samplesToCopy;

        if (x->accumIndex == x->frameSize) {
            ambi_dec_process(x->hAmbi, (const float *const *)x->accumIn, x->outs, x->nSH,
                             x->num_loudspeakers, x->frameSize);
            x->accumIndex = 0;
        }
    }
...
void *decoder_tilde_new(t_symbol *s, int argc, t_atom *argv) {
    ...
    x->frameSize = ambi_dec_getFrameSize();
    x->ins = (t_sample **)getbytes(x->nSH * sizeof(t_sample *));
    x->outs = (t_sample **)getbytes(x->num_loudspeakers * sizeof(t_sample *));

    x->accumIn = (t_sample **)getbytes(x->nSH * sizeof(t_sample *));
    for (int i = 0; i < x->nSH; i++) {
        x->accumIn[i] = (t_sample *)getbytes(x->frameSize * sizeof(t_sample));
    }
    x->accumIndex = 0;
...
void decoder_tilde_free(t_decoder_tilde *x) {
    ...
    if (x->accumIn) {
        for (int i = 0; i < x->nSH; i++) {
            freebytes(x->accumIn[i], x->frameSize * sizeof(t_sample));
        }
        freebytes(x->accumIn, x->nSH * sizeof(t_sample *));
    }
